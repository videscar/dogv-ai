...
def upsert_disposicion_row(
    db: Session, disposicion_id: int, lang: str, source_tag: str | None = None
) -> tuple[int, date] | None:
    """Create (or find) the issue + document row for one disposition, by its internal
    id, in the caller's session. Flushes but does NOT commit. Returns (doc_id,
    pub_date) or None when the detail lacks a usable date.

    Row-creation only: extract/classify/chunk are left to the caller so this can feed
    ...
    if doc is None:
        doc = DogvDocument(
            ...
            doc_tags={source_tag: True} if source_tag else None,
        )
        db.add(doc)
        db.flush()
    return doc.id, pub_date
